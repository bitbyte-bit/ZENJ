...
import { Check, CheckCheck, Play, Pause, Volume2, SmilePlus } from 'lucide-react';
...
const MessageItem: React.FC<MessageItemProps> = ({ message, userName, onReact }) => {
  ...
  return (
    <div className={`flex w-full mb-3 group ${isMe ? 'justify-end' : 'justify-start'}`}>
      <div className={`relative flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] md:max-w-[75%]`}>
        ...
        >
            ...
            <div className="flex items-center gap-3 py-1 pr-6 min-w-[200px]">
              ...
              <div className="flex-1 space-y-1">
                <div className="h-1 bg-white/20 rounded-full relative overflow-hidden">
                   <div className="absolute top-0 left-0 h-full bg-[#00a884] transition-all duration-100" style={{ width: `${(currentTime / duration) * 100 || 0}%` }} />
                </div>
              </div>
          ...
          {message.type === 'text' && (
            <div className="text-[14.5px] leading-relaxed break-words whitespace-pre-wrap pr-12 min-w-[60px]">
              {message.content}
            </div>
          )}
          ...
          <button 
            ...
            className={`absolute -bottom-2 ${isMe ? '-left-8' : '-right-8'} p-1.5 text-[#8696a0] opacity-0 group-hover:opacity-100 transition-opacity hover:text-white`}
          >
            <SmilePlus size={16} />
          </button>
        </div>
          ...
          <div className={`mt-[-8px] flex flex-wrap gap-1 ${isMe ? 'justify-end' : 'justify-start'} z-10`}>
            {Object.entries(message.reactions || {}).map(([emoji, users]) => (
              <div 
                key={emoji} 
                title={users.join(', ')}
                className="bg-[#2a3942] border border-white/10 rounded-full px-1.5 py-0.5 text-xs flex items-center gap-1 shadow-md hover:bg-[#32414a] cursor-help"
              >
                <span>{emoji}</span>
